...
### Thinking UI Render Logic (function `Fnc` - tên minified thay đổi)

Tìm bằng: `{thinking:n,durationMs:e,loading:t=!1,parseHeaders:i=!1}`

```javascript
function Fnc({thinking: n, durationMs: e, loading: t = false, parseHeaders: i = false}) {
  const r = e !== void 0 ? Math.round(e / 1e3) : 0;  // seconds
  const s = !!(n && n.trim().length > 0);              // hasContent

  // Header parsing: title-only content, parseHeaders mode
  const {title: o, hasBodyContent: a, headerCount: l} = s ? Nnc(n) : {...};
  const u = i && o.length > 0;   // parseHeaders && hasTitle
  const d = o.length > 0 && !a;  // titleOnly (has title, no body)
  const m = d || u;              // isHeaderMode

  // f = true → collapsed, NOT expandable
  // BUG (pre-v4): khi loading=true, durationMs=undefined → f=true → v=false → content ẩn!
  // FIX (v4): thêm !t → f=false khi loading → content hiển thị
  const f = !m && !t && (e === void 0 || e <= 0) && r <= 0;  // ← PATCHED: added !t
...
- **Patch B (v4):** Khi `loading=true`, content luôn hiển thị bất kể `thinkingDurationMs`.
  Trước v4, thinking block stuck ở loading animation mà không hiển thị content,
  dù text đã có trong store. Content chỉ hiện khi thinking kết thúc (tất cả cùng lúc).
...
**Root cause:** Hàm render thinking block (`Fnc`) tính `expandable` dựa trên `durationMs`.
...
- User chỉ thấy "Thinking" animation, không thấy thinking content đang stream

**Triệu chứng:**
- Thinking block hiển thị loading animation
- Content đang được append vào store (qua `thinkingWordStreamer`)
- Nhưng UI không render content vì `v = false`
- Khi `handleThinkingCompleted` fire → `durationMs` > 0 → `v = true` → content hiện ra hết cùng lúc
- Thường xảy ra với mọi thinking block, nhưng dễ nhận thấy hơn ở block 2+

**Fix:** Thêm `!t` vào điều kiện `f`:
```
