	...
	if input := root.Get("input"); input.Exists() && input.IsArray() {
		input.ForEach(func(_, item gjson.Result) bool {
				...
				if content := item.Get("content"); content.Exists() && content.IsArray() {
					...
					case "input_file":
						...
						if fileID := contentItem.Get("file_id"); fileID.Exists() {
							contentPart, _ = sjson.SetBytes(contentPart, "file.file_id", fileID.String())
						}
						if fileData := contentItem.Get("file_data"); fileData.Exists() {
							contentPart, _ = sjson.SetBytes(contentPart, "file.file_data", fileData.String())
						}
						if filename := contentItem.Get("filename"); filename.Exists() {
							contentPart, _ = sjson.SetBytes(contentPart, "file.filename", filename.String())
						}
						message, _ = sjson.SetRawBytes(message, "content.-1", contentPart)
						}
						return true
			...
			case "custom_tool_call":
				...
				out, _ = sjson.SetRawBytes(out, "messages.-1", []byte(assistantMessage))
			...
			case "custom_tool_call_output":
				...
				out, _ = sjson.SetRawBytes(out, "messages.-1", []byte(toolMessage))
	...
	if tools := root.Get("tools"); tools.Exists() && tools.IsArray() {
		...
		tools.ForEach(func(_, tool gjson.Result) bool {
			...
			if strict := tool.Get("strict"); strict.Exists() {
				function, _ = sjson.SetBytes(function, "strict", strict.Value())
			}
